...
class quizSession : AppCompatActivity(), View.OnClickListener {
    ...
    private fun answerSheets_data(){
        ...
        val bb = berat_badan.text.toString().toInt()
        val tinggi = tinggi_badan.text.toString().toInt()
        val bmi =(bb)/(tinggi*tinggi)
        val taskid = ref.push().key
        val jawabanUser = AnswerSheets(result.toString(),jumlah_batang,LDL,Data_Tensi, Genders,bmi)
        if (jawabanUser.Smoke==null)
        {
            AnswerSheets("",jumlah_batang,LDL,Data_Tensi,"",bmi)
        }
